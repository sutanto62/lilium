		...
		<div class="mb-4">
			...
			<div class="relative">
				<select
					...
					class="w-full appearance-none rounded-md border border-gray-300 bg-white py-2.5 pl-4 pr-10 text-sm font-medium text-gray-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:focus-visible:ring-gray-400"
				>
	...
	>
		{#if isLoading}
			<div class="flex items-center gap-3 rounded-md border border-gray-200 bg-white px-4 py-6 dark:border-gray-700 dark:bg-gray-800">
				...
				<span class="text-sm text-gray-700 dark:text-gray-300">Memuat petugas...</span>
			</div>
		{:else if formSuccess === false}
			<div class="rounded-md border border-red-200 bg-red-50 px-4 py-4 dark:border-red-800 dark:bg-red-900/20" role="alert">
				<p class="text-sm font-medium text-red-800 dark:text-red-400">Gagal memuat data petugas</p>
				{#if formError}
					<p class="mt-1 text-sm text-red-600 dark:text-red-300">{formError}</p>
				{/if}
			</div>
		{:else if ushers.length === 0}
			<div class="rounded-md border border-gray-200 px-4 py-8 text-center dark:border-gray-700">
				<p class="text-sm text-gray-600 dark:text-gray-400">Belum ada petugas untuk jadwal ini</p>
			</div>
		{:else}
			<table class="w-full border-collapse text-sm">
				<thead>
					<tr class="border-b-2 border-gray-900 dark:border-gray-100">
						<th scope="col" class="py-3 pr-4 text-left text-sm font-semibold text-gray-900 dark:text-gray-100">Nama</th>
						<th scope="col" class="hidden py-3 pr-4 text-left text-sm font-semibold text-gray-900 dark:text-gray-100 lg:table-cell">Zona</th>
						<th scope="col" class="hidden py-3 pr-4 text-left text-sm font-semibold text-gray-900 dark:text-gray-100 lg:table-cell">Posisi</th>
						<th scope="col" class="hidden py-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-100 lg:table-cell">Tugas</th>
					</tr>
				</thead>
				<tbody>
					{#each groupedUshers as group}
						<tr>
							<td
								colspan={4}
								class="border-b border-gray-200 bg-gray-100 py-2 pl-3 text-xs font-semibold uppercase tracking-wide text-gray-600 dark:border-gray-700 dark:bg-gray-700 dark:text-gray-400"
							>
								...
								<td class="py-3 pr-4 text-sm">
									...
									<div class="lg:hidden">
										<div class="flex flex-wrap items-center gap-1.5">
											...
											{#if usher.isPpg}<span class="rounded bg-gray-200 px-1.5 py-0.5 text-xs text-gray-700 dark:bg-gray-600 dark:text-gray-300">PPG</span>{/if}
											{#if usher.isKolekte}<span class="rounded bg-gray-200 px-1.5 py-0.5 text-xs text-gray-700 dark:bg-gray-600 dark:text-gray-300">Kolekte</span>{/if}
										</div>
										<p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{usher.zone} · {usher.position}</p>
									</div>
									<!-- Desktop: name only -->
									<span class="hidden text-gray-900 dark:text-gray-100 lg:inline">{usher.name}</span>
								</td>
								<td class="hidden py-3 pr-4 text-sm text-gray-700 dark:text-gray-300 lg:table-cell">{usher.zone}</td>
								<td class="hidden py-3 pr-4 text-sm text-gray-700 dark:text-gray-300 lg:table-cell">{usher.position}</td>
								<td class="hidden py-3 text-sm text-gray-700 dark:text-gray-300 lg:table-cell">
									{#if usher.isPpg}<span class="mr-1 rounded bg-gray-200 px-1 py-0.5 text-xs text-gray-700 dark:bg-gray-600 dark:text-gray-300">PPG</span>{/if}
									{#if usher.isKolekte}<span class="rounded bg-gray-200 px-1 py-0.5 text-xs text-gray-700 dark:bg-gray-600 dark:text-gray-300">Kolekte</span>{/if}
								</td>
							</tr>
						{/each}
					{/each}
