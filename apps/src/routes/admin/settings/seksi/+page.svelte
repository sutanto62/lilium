<script lang="ts">
	...
	function openDeleteModal(s: Section) {
		selectedSection = s;
...
<Breadcrumb class="mb-4">
	...
	<BreadcrumbItem>Seksi</BreadcrumbItem>
</Breadcrumb>
...
<div class="mb-4">
	...
	{#if sections.length === 0}
		<div class="flex flex-col items-center justify-center rounded-lg border border-gray-200 bg-white p-6 text-center dark:border-gray-700 dark:bg-gray-800">
			<p class="mb-2 text-lg font-medium text-gray-900 dark:text-white">Belum ada seksi.</p>
			<p class="text-sm text-gray-500 dark:text-gray-400">Seksi adalah area fisik di gedung gereja, misal "Nave Utama" atau "Basement".</p>
		</div>
	{:else}
		<Table striped={true} shadow>
			<caption class="bg-white p-5 text-left text-lg font-semibold text-gray-900 dark:bg-gray-800 dark:text-white">
				...
				<p class="mt-1 text-sm font-normal text-gray-500 dark:text-gray-400">
					Seksi adalah pengelompokan area fisik gereja. Setiap seksi berisi satu atau beberapa zona.
				</p>
			</caption>
			<TableHead>
...
<!-- Create / Edit Modal -->
<Modal title={selectedSection ? 'Edit Seksi' : 'Tambah Seksi'} bind:open={showFormModal}>
	<form
		method="POST"
		action={selectedSection ? '?/update' : '?/create'}
		use:enhance={() => {
			isSubmitting = true;
			return async ({ update }) => {
				await update();
...
<Modal title="Hapus Seksi" bind:open={showDeleteModal}>
	{#if selectedSection}
		<P class="mb-4">
			Apakah Anda yakin ingin menghapus seksi <strong>{selectedSection.name}</strong>? Zona di dalam seksi ini tidak akan terhapus.
		</P>
		<form
			method="POST"
			action="?/delete"
